...
runfolder = [3,10]
var = 'h'
print('Compare trispec plots from run ' + str(runfolder))

runpath1 = path+'data/run%04i' % runfolder[0]
D1 = np.load(runpath1+'/analysis/trispec_'+var+'.npy').all()
param1 = np.load(runpath1+'/param.npy').all()

runpath2 = path+'data/run%04i' % runfolder[1]
D2 = np.load(runpath2+'/analysis/trispec_'+var+'.npy').all()
...
c1 = ax1.contourf(D1['k']*kf,D1['f']*sd,np.log10(D1['p']),np.linspace(0,9,51),cmap=cm.thermal,extend='both')
c2 = ax2.contourf(D2['k']*kf,D2['f']*sd,np.log10(D2['p']),np.linspace(0,9,51),cmap=cm.thermal,extend='both')

for D,ax,greyvalue in zip([D1,D2],[ax1,ax2],['0.7','0.3']):

    ax.plot(np.zeros(2),[D['f'][0]*sd,D['f'][-1]*sd],'w',lw=22)

    ax.plot(D['k']*kf,wrossby_min(D['k'])*sd,'c',label=r'$\omega_{Ro,min}(k)$',lw=1.5)
    ax.plot(D['k']*kf,wrossby_max(D['k'])*sd,'c--',label=r'$\omega_{Ro,max}(k)$',lw=1.5)
    ...
    xtick = np.array([40,10,5,3,2,1])*100
...
ax1.set_xlabel(r'wavelength $\lambda = 1/\sqrt{k_x^2 + k_y^2}$ [km]')
ax2.set_xlabel(r'$\lambda$ [km]')
...
ax1.set_title('3D power spectrum of '+varname+': Low resolution $\Delta x = 30$km')
ax2.set_title('High resolution $\Delta x = 7.5$km')
plt.tight_layout()
plt.savefig(path+'/compare/trispec_'+var+'.png')
plt.close(fig)