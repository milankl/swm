...
R = 10          # number of repetitions (from random start dates)

# preallocate
nbins = param['nx']-2   # exclude 
...
while all_startidxs.shape[0]:   # delete from all_startidxs in every loop until empty
    ...
    Hi,xe,ye = np.histogram2d(Xa.flatten(),Ya.flatten(),bins=nbins,range=[[param['dx'],param['Lx']-param['dx']],[param['dy'],param['Ly']-param['dy']]])
    H += Hi
