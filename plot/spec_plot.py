...
ax.loglog(D3['k']*kf,D3['p']/(2*np.pi),'C1',label=r'LR + weak backscatter',lw=1,ls='--')
ax.loglog(D4['k']*kf,D4['p']/(2*np.pi),'C3',label=r'LR + moderate backscatter',lw=1,ls='--')
ax.loglog(D5['k']*kf,D5['p']/(2*np.pi),'C5',label=r'LR + strong backscatter',lw=1,ls='--')
...
ax.text(1/1900,5,'$L_{Ro}^{max}$',rotation=90,fontsize=12)
ax.text(1/590,5,'$L_{Ro}^{min}$',rotation=90,fontsize=12)
...
plt.tight_layout()
plt.savefig(outpath+'plots/spec_eke.pdf')
plt.close(fig)