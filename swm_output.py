...
def output_nc_ini():
    ...
    nce['file'] = Dataset(param['output_runpath']+'/e.nc','w',format=ncformat)

    # write general attributes
    for ncfile in [ncu,ncv,nceta,nce]:
    ...
    nce['xdim'] = nce['file'].createDimension('x',param['nx'])
    nce['ydim'] = nce['file'].createDimension('y',param['ny'])
    nce['tdim'] = nce['file'].createDimension('t',None)

    # create variables
    p = 'f4' # 32-bit precision storing, or f8 for 64bit
    for ncfile,var in zip([ncu,ncv,nceta,nce],['u','v','eta','e']):
    ...
    # write dimensions
    for ncfile,var in zip([ncu,ncv,nceta,nce],['u','v','T','T']):
